...
Func chkShieldStatus($bChkShield = True, $bForceChkPBT = False)
	...
	If ($ichkSinglePBTForced = 0 And ($iChkBotStop = 1 And $icmbBotCond >= 19 and $icmbBotCond < 22) = False) Or Not(IsMainPage()) Then Return ; return if force single PB or wait for shield not enabled.

	Local $Result, $iTimeTillPBTstartSec, $ichkTime = 0, $ichkSTime = 0, $ichkPBTime = 0

	If $bChkShield Or $aShieldStatus[0] = "" Or $aShieldStatus[1] = "" Or $aShieldStatus[2] = "" Or $sPBStartTime = "" Or $bGForcePBTUpdate = True and $icmbBotCond <> 22 Then ; almost always get shield information
		...
		If IsArray($Result) Then
			...
			If $iChkBotStop = 1 And $icmbBotCond >= 19 and $icmbBotCond < 22 Then ; is Halt mode enabled and With Shield selected?
